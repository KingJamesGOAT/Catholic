...
// Import data
import { COUNCILS, SAINTS, POPES, WRITINGS, KINGS, TimelineEvent, TIMELINE_UI } from '../lib/data/timelineData'; 
...
export default function HistoryTimeline() {
  ...
  const allEvents = useMemo(() => {
    ...
    return [
      ...
      ...kings.map(e => ({ ...e, type: 'king' as const }))
  ...
  useEffect(() => {
    ...
    if (query.startsWith('/')) {
      ...
      const commands = [
        ...
        { isCommand: true, label: 'Filter: King', value: 'king', type: 'category' },
      ];
  ...
  const getColors = (type: string) => {
    switch(type) {
      ...
      // FIX: Changed to standard Tailwind classes to ensure solid background
      case 'king': return { bg: 'bg-purple-600', border: 'border-purple-400', hover: 'hover:bg-purple-500' };
  ...
  const getIcon = (type: string) => {
    ...
    switch(type) {
      ...
      case 'king': return <Sword size={size} className="text-purple-100" />;
    ...
    <div className="w-full min-h-screen bg-[#050505] text-white pt-24 px-4 pb-12 flex flex-col gap-6">
      ...
      <div className="container mx-auto w-full max-w-7xl bg-[#121212] border border-gray-800 p-4 rounded-xl shadow-lg">
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-4">
           ...
           {(['council', 'saint', 'pope', 'writing', 'king'] as const).map(type => (
