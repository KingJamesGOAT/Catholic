    ...
    >
      ...
      <div className="absolute inset-0 bg-black/20 group-hover:bg-black/40 transition-colors flex items-center justify-center">
        <div className="w-12 h-12 md:w-16 md:h-16 bg-white/90 rounded-full flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform">
          <Play className="ml-1 text-black fill-black w-5 h-5 md:w-8 md:h-8" />
        </div>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-black pt-32 md:pt-24 pb-20">
      ...
      <div className="container mx-auto max-w-6xl px-4">
        ...
        >
          ...
          {/* --- START ADDED VIDEO SECTION --- */}
          <motion.div 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.2 }}
            className="grid grid-cols-2 gap-3 md:gap-6 max-w-4xl mx-auto mt-8 mb-8"
          >
            {[
              { id: "CK1_22AjdIQ", title: "Part 1: How St. Ignatius 'Red Pilled' Me on the Catholic Church" },
              { id: "uYUm-eAP5v0", title: "Part 2: How St. Ignatius 'Red Pilled' Me on the Catholic Church" }
            ].map((video) => (
              <div key={video.id} className="flex flex-col gap-2">
                <div className="aspect-video rounded-lg overflow-hidden border border-gray-800 shadow-lg bg-gray-900 relative">
                  <VideoFacade id={video.id} title={video.title} />
                </div>
                <h3 className="text-xs md:text-sm font-medium text-gray-400 text-center leading-tight">
                  {video.title}
                </h3>
              </div>
            ))}
          </motion.div>
          {/* --- END ADDED VIDEO SECTION --- */}
